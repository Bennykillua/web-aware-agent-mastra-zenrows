...
**402 with `AUTH010`** — `extract=auto` is not enabled for that domain on your account. Contact Zenrows support to request access, or use the Markdown path instead since this feature is still in beta. Alternatively, check out the `extract=auto` docs [here](https://docs.zenrows.com/extract/setup) for the list of approved URLs.
